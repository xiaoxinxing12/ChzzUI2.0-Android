 ...
 * @param <M> 适配的数据类型
 */
public abstract class CHZZDataBindingAdapter<M> extends RecyclerView.Adapter<CHZZDataBindingHolder> {
    ...
    public CHZZDataBindingAdapter(RecyclerView recyclerView, int itemLayoutId, View headView, View footView, boolean isBinding) {
        mRecyclerView = recyclerView;
        mContext = mRecyclerView.getContext();
        mItemLayoutId = itemLayoutId;
        mData = new ArrayList<>();
    ...
    public CHZZDataBindingHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (isBinding) {
            ...
            if (viewType < 1 && mHeadViews != null) {
                //头部
                viewHolder = new CHZZDataBindingHolder(mRecyclerView, mHeadViews, mOnRVItemClickListener, mOnRVItemLongClickListener);
            } else if (viewType > mData.size() && mFootViews != null && mHeadViews != null) {
                //底部
                viewHolder = new CHZZDataBindingHolder(mRecyclerView, mFootViews, mOnRVItemClickListener, mOnRVItemLongClickListener);
            } else if (viewType >= mData.size() && mFootViews != null && mHeadViews == null) {
                //底部
                viewHolder = new CHZZDataBindingHolder(mRecyclerView, mFootViews, mOnRVItemClickListener, mOnRVItemLongClickListener);
            } else {
                //正常
                viewHolder = new CHZZDataBindingHolder(mRecyclerView, LayoutInflater.from(mContext).inflate(mItemLayoutId, parent, false), mOnRVItemClickListener, mOnRVItemLongClickListener);
    ...
    public abstract void setBindingItem(ViewGroup parent);
}