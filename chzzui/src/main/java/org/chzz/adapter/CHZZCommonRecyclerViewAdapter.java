 ...
 * @param <M> 适配的数据类型
 */
public abstract class CHZZCommonRecyclerViewAdapter<M> extends RecyclerView.Adapter<CHZZRecyclerViewHolder> {
    ...
    public CHZZCommonRecyclerViewAdapter(RecyclerView recyclerView, int itemLayoutId) {
    ...
    public CHZZCommonRecyclerViewAdapter(RecyclerView recyclerView, int itemLayoutId, View headView, View footView) {
        mRecyclerView = recyclerView;
        mContext = mRecyclerView.getContext();
        mItemLayoutId = itemLayoutId;
        mData = new ArrayList<>();
    ...
    public CHZZRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType < 1 && mHeadViews != null) {
            //头部
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, mHeadViews, mOnRVItemClickListener, mOnRVItemLongClickListener);
        } else if (viewType > mData.size() && mFootViews != null && mHeadViews != null) {
            //底部
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, mFootViews, mOnRVItemClickListener, mOnRVItemLongClickListener);
        } else if (viewType >= mData.size() && mFootViews != null && mHeadViews == null) {
            //底部
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, mFootViews, mOnRVItemClickListener, mOnRVItemLongClickListener);
        } else {
            //正常
            viewHolder = new CHZZRecyclerViewHolder(mRecyclerView, LayoutInflater.from(mContext).inflate(mItemLayoutId, parent, false), mOnRVItemClickListener, mOnRVItemLongClickListener);
